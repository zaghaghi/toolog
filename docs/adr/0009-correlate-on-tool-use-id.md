...
- **OTEL only** — the call was **rejected**. Denied calls leave no transcript record whatsoever, so
  this is the *only* way a refusal is ever observed.
...
5. **`toolog verify` reports divergence as findings**, with a per-session completeness figure:
   OTEL-only calls listed as rejections, transcript-only calls listed as gaps.
...
- Rejected calls are captured as first-class rows — the security surface transcripts cannot see.
